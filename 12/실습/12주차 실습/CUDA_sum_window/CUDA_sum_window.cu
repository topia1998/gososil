...
__global__ void Sum_n_elements_Kernel_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	if (thread_id < Nf) {
		if (id >= Nf) shared_buffer[thread_id] = d_ArrayElements[id - Nf];
		else shared_buffer[thread_id] = 0;
	}
	if (thread_id >= BLOCK_SIZE - Nf) {
		if (id + Nf >= N) shared_buffer[thread_id + 2 * Nf] = 0;
		else shared_buffer[thread_id + 2 * Nf] = d_ArrayElements[id + Nf];
	}
