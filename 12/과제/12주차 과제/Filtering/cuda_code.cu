...
__global__ void Gaussian_kernel_shared(INOUT unsigned char *d_bitmaps, OUT unsigned char *d_Gaussian, long width, long height) {
	...
	int row = id / width;
	int col = id % width;
	int sharedRowSize = BLOCK_SIZE + 4;
	int i;
	if (thread_id == 0)
	{
		for (i = 0; i < 2; i++)
		{
			if (col + i < 2)
			{
				...
				sharedBuffer[i + sharedRowSize * 2] = 0;
			...
			else
			{
				...
				sharedBuffer[i + sharedRowSize * 2] = d_bitmaps[id + i - 2];

				if (row + 1 < height)
					sharedBuffer[i + sharedRowSize * 3] = d_bitmaps[(row + 1) * width + col + i - 2];
				else
					sharedBuffer[i + sharedRowSize * 3] = 0;

				if (row + 2 < height)
					sharedBuffer[i + sharedRowSize * 4] = d_bitmaps[(row + 2) * width + col + i - 2];
				else
					sharedBuffer[i + sharedRowSize * 4] = 0;
			}
		}
	...
	if (thread_id == BLOCK_SIZE - 1)
	{
		for (i = 1; i <= 2; i++)
		{
			if (col + i >= width)
			{
				...
				sharedBuffer[thread_id + 2 + i + sharedRowSize * 2] = 0;
			...
			else
			{
				...
				sharedBuffer[thread_id + 2 + i + sharedRowSize * 2] = d_bitmaps[id + i];

				if (row + 1 < height)
					sharedBuffer[thread_id + 2 + i + sharedRowSize * 3] = d_bitmaps[(row + 1) * width + col + i];
				else
					sharedBuffer[thread_id + 2 + i + sharedRowSize * 3] = 0;

				if (row + 2 < height)
					sharedBuffer[thread_id + 2 + i + sharedRowSize * 4] = d_bitmaps[(row + 2) * width + col + i];
				else
					sharedBuffer[thread_id + 2 + i + sharedRowSize * 4] = 0;
			}
		}
	...
	sharedBuffer[(thread_id + 2) + sharedRowSize * 2] = d_bitmaps[id];

	if (row + 1 < height)
		sharedBuffer[thread_id + 2 + sharedRowSize * 3] = d_bitmaps[(row + 1) * width + col];
	else
		sharedBuffer[thread_id + 2 + sharedRowSize * 3] = 0;
	if (row + 2 < height)
		sharedBuffer[thread_id + 2 + sharedRowSize * 4] = d_bitmaps[(row + 2) * width + col];
	else
...
float Do_Gaussian_on_GPU(IN unsigned char *p_bitmaps, OUT unsigned char *p_Gaussian, long width, long height, int Shared_flag)
{
	...
	dim3 gridDim(width * height / BLOCK_SIZE, 1);
	CHECK_TIME_INIT_GPU();
