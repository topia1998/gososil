...
void Mat_Vec_Multiply_GPU(float *p_VecX, float *p_MatA, float *p_VecY_G) {
	...
	CUDA_CALL(cudaMalloc(&dVecX, size))

	CUDA_CALL(cudaMemcpy(dVecX, p_VecX, size, cudaMemcpyHostToDevice))

	size = ELEM_PER_VECTOR * ELEM_PER_VECTOR * sizeof(float);
	CUDA_CALL(cudaMalloc(&dMatA, size))
	CUDA_CALL(cudaMemcpy(dMatA, p_MatA, size, cudaMemcpyHostToDevice))


	size = n * ELEM_PER_VECTOR * sizeof(float);
	CUDA_CALL(cudaMalloc(&dVecY, size))

	size = ELEM_PER_VECTOR;
	...
	CHECK_TIME_INIT_GPU()
	CHECK_TIME_START_GPU()
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (dVecY, dVecX, dMatA, size);
	CHECK_TIME_END_GPU(device_time)
	CHECK_TIME_DEST_GPU()
	...
	// any errors encountered during the launch.
	CUDA_CALL(cudaDeviceSynchronize())
	CUDA_CALL(cudaMemcpy(p_VecY_G, dVecY, size, cudaMemcpyDeviceToHost))
